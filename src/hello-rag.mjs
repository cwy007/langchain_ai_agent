...
for (const question of questions) {
  ...
  // 使用 similaritySearchWithScore 方法获取文档和相似度分数
  const docsWithScores = await vectorStore.similaritySearchWithScore(question, 3);
  console.log('docsWithScores: ', JSON.stringify(docsWithScores, null, 2));

  // 打印用到的文档和相似度评分
  console.log("\n 检索到的相关文档和相似度评分:");
  for (const [doc, score] of docsWithScores) {
  ...
  // 构建 prompt，包含问题和检索到的文档内容
  const context = docsWithScores.map(([doc, score], i) => `[片段${i + 1}]: ${doc.pageContent}`).join("\n\n======\n\n");
