...
console.log(`Total documents after splitting: ${splitDocuments.length}`);

const enc = getEncoding('cl100k_base');
splitDocuments.forEach((doc, index) => {
  console.log(doc)
  const tokenCount = enc.encode(doc.pageContent).length;
  console.log(`Document ${index + 1} token count: ${tokenCount}`);
  const characterCount = doc.pageContent.length;
  console.log(`Document ${index + 1} character count: ${characterCount}`);
});