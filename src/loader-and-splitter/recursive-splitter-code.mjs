...
const jsCodeDocument = new Document({
  pageContent: jsCode
});
...
const splitCodeDocuments = await codeSplitter.splitDocuments([jsCodeDocument]);

splitCodeDocuments.forEach((doc, index) => {
  console.log(`--- Code Chunk ${index + 1} ---`);
  console.log(doc);
  console.log(`Character count: ${doc.pageContent.length}`);
});