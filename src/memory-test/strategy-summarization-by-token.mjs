...
async function testSummarization() {
  ...
  const keepRecentTokens = 80; // 保留最近 token 数量（约占总数的 40%）
  ...
  console.log(`当前消息总 token 数量: ${totalTokens}/${maxTokens}`);

  // 如果消息总 token 数量超过限制，则进行总结
  if (totalTokens > maxTokens) {

    // 从后往前累加消息，保留最近的消息直到达到 keepRecentTokens 的限制
    ...
    for (let i = allMessages.length - 1; i >= 0; i--) {
      ...
      if (recentTokens + tokenCount <= keepRecentTokens) {
        recentMessages.unshift(msg); // 保持消息顺序
        recentTokens += tokenCount;
      } else {
        break; // 超过保留的最近 token 数量，停止添加到 recentMessages
      }
    }
    const messagesToSummarize = allMessages.slice(0, allMessages.length - recentMessages.length);
    const tokensToSummarize = countTokens(messagesToSummarize, enc);

    console.log(`\n历史消息过多，开始总结...`);
    console.log(`需要总结的消息数量: ${messagesToSummarize.length} (${tokensToSummarize} tokens)`);
    console.log(`保留的最近消息数量: ${recentMessages.length} (${recentTokens} tokens)`);

    // 总结将被丢弃的旧消息
    const summary = await summarizeMessages(messagesToSummarize);
    console.log(`\n总结内容: ${summary.content} (${enc.encode(summary.content).length} tokens)`);
    ...
    console.log(`总结后的消息内容:`, allMessages.map((msg, index) => {
      ...
      return `${msg.constructor.name}: ${msg.content} (${tokens} tokens)`;
    }).join('\n'));
  } else {
    console.log(`当前消息总 token 数量未超过限制，无需总结。`);
  }
}
