// 智能导入 - 使用 AI 从文本中提取结构化数据，并导入到 mysql 数据库中
...
async function extractAndInsert(text) {
  ...
  try {
    ...
    const prompt = `请从以下文本中提取所有好友信息，文本中可能包含一个或多个人的信息。请将每个人的信息分别提取出来，返回一个数组。数据是 json 格式。
...
2. 每个对象包含以下字段：
   - name 姓名：提取文本中的人名
   - gender 性别：提取性别信息（男/女）
   - birth_date 出生日期：如果能找到具体日期最好，否则根据年龄描述估算（格式：YYYY-MM-DD）
   - company 公司：提取公司名称
   - title 职位：提取职位/头衔信息
   - phone 手机号：提取手机号码
   - wechat 微信号：提取微信号
