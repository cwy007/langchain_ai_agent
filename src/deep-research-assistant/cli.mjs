...
const hideLangSmithErrors = process.env.HIDE_LANGSMITH_ERRORS !== "false";
...
function isLangSmithNoise(line) {
  return /^Error in handler/.test(line.trim());
}

function installLangSmithErrorFilter() {
  const write = process.stderr.write.bind(process.stderr);
  let buffer = "";

  function flush() {
    if (buffer && !isLangSmithNoise(buffer)) write(buffer);
    buffer = "";
  }
  ...
  process.stderr.write = (chunk, encoding, callback) => {
    ...
    buffer += Buffer.isBuffer(chunk) ? chunk.toString(encoding) : String(chunk);
    const lines = buffer.split(/(?<=\n)/);
    buffer = lines.pop() ?? "";

    for (const line of lines) {
      if (!isLangSmithNoise(line)) write(line);
    }
...
if (hideLangSmithErrors) {
  installLangSmithErrorFilter();
}
...
// node cli.mjs "调研国家统计局公开的 2025 年省级地区生产总值（GDP）数据：提取GDP总量前6名省份的具体数值及同比增速，计算6省GDP总和、各省占全国GDP比重，并按增速从高到低排名"